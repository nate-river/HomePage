...
String.implement({
    ...
    j.setDocument = function(w) {
        ...
        p.removeChild(z);
        z = s = p = null;
        o.getAttribute = (o.isHTMLDocument && r) ?
        function(D, B) {
        ...
        o.hasAttribute = (y && this.isNativeCode(y.hasAttribute)) ?
        function(C, B) {
        ...
        o.contains = (y && this.isNativeCode(y.contains)) ?
        function(B, C) {
            return B.contains(C)
        ...
        if (typeof D == "string") {
            ...
            if (D == null) {
                ...
                if (D.Slick) {
                    ...
                    if (this.contains(q.documentElement || q, D)) { (B) ? B.push(D) : B = D;
                        return B
                    } else {
                        return B
                    }
...
if (window.$$ == null) {
    ...
    Element.implement({
        set: function(r, q) {
            var p = Element.Properties[r]; (p && p.set) ? p.set.call(this, q) : this.setProperty(r, q)
        }.overloadSetter(),
        get: function(q) {
        ...
        setProperty: function(q, r) {
            ...
            var p = g[q]; (p) ? this[p] = r: (d[q]) ? this[q] = !!r: this.setAttribute(q, "" + r);
            return this
        },
        ...
        removeProperty: function(q) {
            q = o[q] || q;
            var p = g[q]; (p) ? this[p] = "": (d[q]) ? this[q] = false: this.removeAttribute(q);
            return this
        },
...
Element.Properties.html = (function() {
    ...
    Element.ShortStyles = {
        ...
        var g = Element.Styles; ["margin", "padding"].each(function(m) {
            var n = m + l;
            k[m][n] = g[n] = "@px"
        });
        var j = "border" + l;
...
Event.implement({
    ...
    var e = this.ContentContainer = new Class({
        ...
        setURL: function(m, o, n) {
            ...
            var l = this.getContentClassForURL(m, n);
            this.content = new l(this, n)
        },
        getContentClassForURL: function(l, m) {
            return (m && m.vimeo) ? h: e.isMovieURL(l) ? j: e.isImageURL(l) ? k: (m && m.injectContent) ? i: c
        },
        getContentSizeForProperties: function(q, t, l) {
            var s = q.imageWidth || q.pageWidth || b;
            var p = q.imageHeight || q.pageHeight || l;
            var o = q.minimumImageScale || a;
            var r = l - this.bottomSpacer.height;
            if (q.imageHeight && p > r && !q.noSquish) {
                var m = r / p;
                if (m > o) {
                    s = Math.round(s * m);
                    p = Math.round(p * m)
        ...
        preload: function(m) {
            m = m || this.preloadURL;
            if (m && !this.preloadedURLs[m]) {
                var l = this.getContentClassForURL(m);
                l.preloadWithURL(m);
                this.preloadedURLs[m] = true
            }
            this.preloadURL = null
        }
    });
    e.getBrowserSpecificURL = function(l) {
        if (this.isMovieURL(l)) {
            return this.getBrowserSpecificMovieURL(l)
        }
        return l
    };
    e.getPreviewURL = function(l) {
        if (this.isMovieURL(l)) {
            return this.getMoviePreviewURL(l)
        }
        return l
    };
    var k = new Class({
        ...
        initialize: function(n, m) {
            this.parent(n);
            var l = this.container = this.getAncestorWithClass(e);
            this.setContentsURLAndSize(l.url, l.width, l.height);
    ...
    k.preloadWithURL = function(l) {
        var m = new Image();
        m.src = l
    };
    e.isImageURL = function(l) {
        return !! (l.match(/\.jpg$/) || l.match(/\.png$/))
    };
    var j = new Class({
        Extends: BVLayer,
        initialize: function(o, n) {
            this.parent(o);
            var l = this.container = this.getAncestorWithClass(e);
            var m = e.getMoviePreviewURL(l.url);
            this.setContentsURLAndSize(m, l.width, l.height);
            if (f) {
            ...
            this.spinner.setPosition(0.5 * (l.width - this.spinner.width), -0.5 * (l.height - this.spinner.height));
    ...
    j.preloadWithURL = function(l) {};
    e.isMovieURL = function(l) {
        return !! (l.match(/\.mov$/) || l.match(/\.mp4$/) || l.match(/\.ogv$/) || l.match(/\.webm$/))
    };
    e.getBrowserSpecificMovieURL = function(l) {
        var m = Browser.firefox3 ? ".ogv": (Browser.firefox || Browser.chrome) ? ".webm": null;
        if (m) {
            l = l.replace(/\.\w+$/, m)
        }
        return l
    };
    e.getMoviePreviewURL = function(l) {
        return l.replace(/\.\w+$/, ".jpg")
    };
    var i = new Class({
        Extends: BVLayer,
        initialize: function(n, m) {
        ...
        requestWasSuccessful: function(n) {
            var l = n.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
            if (!l) {
                return
            }
            var m = l[1];
            this.element.set("html", m);
            if (this.preloadTimer) {
                clearTimeout(this.preloadTimer)
            }
            this.preloadTimer = this.container.preload.delay(1000, this.container)
    ...
    i.preloadWithURL = function(l) {
        c.preloadWithURL(l)
    };
    var c = new Class({
        initialize: function(l, o) {
            this.container = l;
            var p = l.getGlobalPosition();
            var m = o.pageColor;
            if (m === undefined) {
                m = "fff"
            }
            var n = l.url;
            if (o.queryWindowSize) {
                n += "?width=" + this.container.width + "&height=" + Root.root.height
            }
            ...
            this.iframeHeight = l.height;
            this.iframeShouldUpdateHeight = !o.pageHeight;
            this.iframeHeightCanChange = !!o.pageHeightCanChange;
            this.iframeShouldScrollToEnd = (o.pageHeightCanChange === "scrollToEnd");
        ...
        updateIframeVisibilityAndHeight: function() {
            ...
            if (this.iframeShouldUpdateHeight) {
                var l = this.container.height;
                var o = this.iframe.contentDocument;
                if (o) {
                    var m = o.body;
                    var n = o.documentElement;
                    if (m) {
                        l = Math.max(l, m.scrollHeight, m.offsetHeight, m.clientHeight)
                    }
                    if (n) {
                        l = Math.max(l, n.scrollHeight, n.offsetHeight, n.clientHeight)
                    }
                }
                if (l != this.iframeHeight) {
                    this.iframeHeight = l;
                    this.iframe.set("height", l);
                    if (this.iframeIsLoaded && this.iframeShouldScrollToEnd) { (function() {
    ...
    c.preloadWithURL = function(l) {
        ...
        function m(s) {
            var r = s.match(/preload-image href="([^"]+)"/);
            if (!r) {
                return
            }
            var t = new Image();
    ...
    var h = new Class({
        initialize: function(l, m) {
            this.container = l;
            var o = m.vimeo;
            var n = l.getGlobalPosition();
            this.iframe = new IFrame({
                src: "http://player.vimeo.com/video/" + o + "?title=0&byline=0&portrait=0",
                styles: {
                    position: "absolute",
                    left: "" + n.x + "px",
                    top: "" + ( - n.y) + "px",
    ...
    var g = new Class({
        ...
        setSpinning: function(l) {
            if (l == this.isSpinning) {
                return
            }
            this.isSpinning = l;
            this.setHidden(!l);
        ...
        updateSpin: function() {
            var m = Date.now();
            var l = m - this.lastTimestamp;
            this.lastTimestamp = m;
            this.setRotation(this.rotation + 8 * l / 1000)
        }
    })
})();
...
var lerp = function(d, c, e) {
    ...
    var a = this.Site = new Class({
        ...
        setZoomedSegment: function(j, f, i) {
            ...
            if (!l && k) {
                ...
                BVLayer.animate(this.zoomDuration,
                function() {
                ...
                if (l && !k) {
                    ...
                    BVLayer.animate(this.unzoomDuration,
                    function() {
    ...
    var a = new Class({
        ...
        getMagicCode: function() {
            return ""
        }
    })
})(); (function() {
    ...
    var b = this.SiteStripSegment = new Class({
        ...
        initialize: function(d, j) {
            ...
            this.setTouchable(true);
            this.element.setStyle("cursor", "pointer");
            this.properties = this.site.mergePropertiesFromElement(this.strip.properties, j);
            this.parseSegmentElement(j);
            var e = this.strip.stripScale;
            var g = this.properties.widthScale || 1;
            this.setSize(this.strip.segmentSize.width * g, this.strip.segmentSize.height);
            var k = this.properties.filmEdges ? -29 : -14;
            var l = {
                x: Math.round(5 * e),
                y: Math.round(k * e)
            };
            var m = {
                width: Math.round((218 * g - 10) * e),
                height: Math.round(170 * e)
            };
            var i = {
                x: Math.round(4 * e),
                y: Math.round((k + 1) * e)
            ...
            var h = {
                width: Math.round((220 * g - 10) * e),
                height: Math.round(172 * e)
            };
            this.film = new BVLayer(this);
            var f = "Images/FilmMiddle" + (this.properties.filmEdges ? "": "Solid") + ".png";
            this.film.setContentsURLAndSize(f, this.getSize());
            this.contentClip = new BVLayer(this);
            this.contentClip.setPosition(l);
            this.contentClip.setSize(m.width, m.height - 1);
            this.window = new BVLayer(this);
            this.window.setContentsURLAndSize("Images/Window.png", m);
            this.window.setPosition(l);
        ...
        parseSegmentElement: function(g) {
            var f = g.getElement("a");
            this.name = f.get("id");
            this.url = ContentContainer.getBrowserSpecificURL(f.get("href"));
            var i = g.getElement(".display") || f;
            this.displayURL = ContentContainer.getBrowserSpecificURL(i.get("href"));
            this.contentClickURL = (this.displayURL == this.url) ? null: this.url;
            var d = g.getElement(".preview") || i;
            this.previewURL = ContentContainer.getPreviewURL(d.get("href"));
            var j = this.url.match(/\.com\/(.+)/);
            this.localURL = (j && j[1]) ? j[1] : this.url;
            this.localURL = this.localURL.replace(/\/$/, "");
            this.urlIndex = (this.properties.urlIndex || 0) * 1;
            var k = g.getElement(".title");
            this.titleString = k ? k.get("html") : "";
            var e = g.getElement(".subtitle");
            this.subtitleString = e ? e.get("html") : "";
            var h = g.getElement(".caption");
            this.captionString = h ? h.get("html") : ""
        },
        addContent: function() {
    ...
    var b = this.SitePageSet = new Class({
        ...
        getAvailableHeightForPage: function(f) {
            var e = this.tocHeight * Math.max(0.51, f.segment.strip.stripScale);
        ...
        setZoomedPage: function(f) {
            ...
            if (!g) {
                ...
                if (!f) {
                    BVLayer.animate(this.site.unzoomDuration,
                    function() {
                        this.updatePagePositionsAroundPage(g, false);
                        this.backgroundGradient.setY( - this.site.height)
                    },
                    this);
                    e(this.site.unzoomDuration + 200,
                    function() {
                        this.hideAllPages();
                        this.backgroundGradient.setHidden(true)
                    },
